...
module Utils = struct
  ...
  let render_cursor ~width cursor =
    ...
    let left, middle, right =
      ...
      | false -> rope, None, Rp.empty in
    let middle =
      match middle with
      | None -> I.uchar A.empty (Uchar.of_char ' ') 1 1
      | Some uchar -> I.uchar A.empty uchar 1 1 in
...
module Command = struct
  let handler ~cursor ~(hook : ?mode:_ -> _ -> unit) state = function
    ...
    | `Uchar uchr, [] ->
      ...
      hook ~mode:(`Command cursor) state
      ; `Handled
    | `Escape, [] -> hook ~mode:`Normal state ; `Handled
    | `Backspace, [] ->
      let position = Rp.Cursor.position cursor in
      (if position > 0 then
...
module User_prompt = struct
  let render ~cursor ~y ~w state mode =
    ...
    let text, position =
      Utils.render_cursor ~width:(max 0 (w - 3)) state.cursor in
...
module Command_prompt = struct
  let render ~cursor ~y ~w mode =
    match mode with
    | `Command cursor' ->
      let text, position = Utils.render_cursor ~width:(max 0 (w - 3)) cursor' in
      Lwd.set cursor (position + 2, y + 1)
      ; I.hcat
          [I.uchars A.empty [|Uchar.of_char ' '; Uchar.of_int 0x0589|]; text]
...
let make ~command ~message cursor status mode _w =
  ...
  let update_position ~x:_ ~y ~w ~h:_ () =
    ...
    if y' <> y || w' <> w then Lwd.set position (y, w) in

  let* prompts =
    let+ state = Lwd.get state
    and+ position = Lwd.get position
    and+ mode = Lwd.get mode in
    update_prompts state mode position in
