  ...
  let tabs =
    ...
    | `Console ->
      if idx = t.active then
        I.[strf ~attr:A.(fg red) "["; strf "console"; strf ~attr:A.(fg red) "]"]
        |> I.hcat
      else I.strf " console "
    | `Window title ->
      if idx = t.active then
        I.
          [
            strf ~attr:A.(fg yellow) "["; strf "%s" title
          ; strf ~attr:A.(fg yellow) "]"
          ]
        |> I.hcat
      else I.strf " %s " title in
...
let make tabs =
  ...
  let* document =
    ...
    Ui.keyboard_area (handler ~hook tabs) (render tabs) in
  Lwd.return document
